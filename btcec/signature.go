...
// BigIntConverter 用于转换字符串和big.Int之间
type BigIntConverter struct {
	TotalBytes int // 转换为big.Int时总字节数（应为32）
}

// NewBigIntConverter 创建一个新的BigIntConverter
func NewBigIntConverter(totalBytes int) *BigIntConverter {
...
func (bc *BigIntConverter) StringToBigInt(msg string) *big.Int {
	// 将字符串转换为字节切片
	bytes := []byte(msg)

	// 截断或填充字节，使总长度为 31 字节
	// 我们需要保留一个字节的空间给前缀
	if len(bytes) > bc.TotalBytes-1 {
		bytes = bytes[:bc.TotalBytes-1]
	} else if len(bytes) < bc.TotalBytes-1 {
		padding := make([]byte, bc.TotalBytes-1-len(bytes))
		bytes = append(bytes, padding...)
	}

	// 添加一个固定的非零前缀字节以确保 big.Int 是 256 位
	prefix := []byte{0xFF}
...
// BigIntToString 从 *big.Int 还原为字符串的函数
func (bc *BigIntConverter) BigIntToString(bi *big.Int) string {
	// 将 big.Int 转换为字节切片
	bytes := bi.Bytes()
